 ...
 *
 * NOTE: this runs as a persistent Node process (app.listen below) —
 * it is NOT currently set up to deploy as a Vercel serverless
 * function. Doing that properly means restructuring this file (no
 * app.listen, export a request handler instead, and a per-request
 * or cached MongoDB connection instead of one made at startup). Ask
 * if/when you're ready to actually deploy this backend somewhere.
 */
...
// GET /api/admin/verify — used only by login.html to confirm the
// token + password pair is correct before redirecting to the
